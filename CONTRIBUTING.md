# Contributing to AgentSwarm: Join the Syndicate

We welcome contributions from developers, researchers, and blockchain enthusiasts who wish to enhance AgentSwarm. By contributing, you help us build a more robust, intelligent, and efficient system for navigating the Solana ecosystem.
...
There are several ways you can contribute to AgentSwarm:
...
1.  **Fork the Repository**: Start by forking the `ereezyy/AgentSwarm` repository to your own GitHub account.
2.  **Create a Feature Branch**: Create a new branch from `main` for your changes. Use a descriptive name (e.g., `feature/add-new-llm`, `bugfix/fix-mev-logic`).
...
7.  **Commit Your Changes**: Write clear, concise, and descriptive commit messages. We follow the [Conventional Commits](https://www.conventionalcommits.org/en/v1.0.0/) specification.
    ...
    Example:
    ```bash
    git commit -m "feat: Integrate new LLM provider for enhanced analysis"
    ```
...
9.  **Create a Pull Request**: Finally, open a pull request from your forked repository to the `main` branch of the original `ereezyy/AgentSwarm` repository. Provide a detailed description of your changes and why they are necessary.
