...
async function withRetry(fn, maxRetries = 3, delayMs = 1000) {
  for (let i = 0; i < maxRetries; i++) {
    try {
      ...
      console.warn(`[Network] Attempt ${i + 1} failed. Retrying in ${backoff}ms... (${error.message || 'Unknown'})`);
    ...
    while (this.currentCapital < this.minCapitalThreshold && this.operationStatus === 'RUNNING') {
      try {
        ...
        if (balance !== null && balance < 0.005) {
          console.warn(`[SeedFundingAgent] Wallet guard triggered. Balance too low: ${balance} SOL`);
