...
try {
    if (process.env.SOLANA_PRIVATE_KEY) {
        try {
            // Attempt to parse as JSON array (e.g., from phantom or local test keys)
            const secretKey = Buffer.from(JSON.parse(process.env.SOLANA_PRIVATE_KEY));
            keypair = Keypair.fromSecretKey(secretKey);
            WALLET = keypair.publicKey.toString();
        } catch (e) {
            // Fallback to parsing as hex string
...
async function checkBalance() {
    ...
    return runWithRetry(async () => {
        ...
        try {
            ...
            console.log(chalk.yellow(`[SCAVENGER #${id}]: Primary RPC failed (${e.message}), using fallback...`));
...
async function sweepDust() {
    if (!keypair) {
        console.log(chalk.gray(`[SCAVENGER #${id}]: SIMULATION_MODE: Skipping rent sweep.`));
        return;
    }
    try {
